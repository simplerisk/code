 ...
 *
 * The status check, the derived pending-risk generation, and the terminal
 * status update all run inside one transaction, guarded by a `SELECT ...
 * FOR UPDATE` lock on the assessment row. Without this, two concurrent
 * completion requests for the same id can both observe 'in_progress' before
 * either commits, and both generate their own duplicate set of pending
 * risks (HackerOne #3961544 / SR-2096). With the lock, the second request
 * blocks until the first commits, then re-reads the row as 'completed'
 * below and no-ops instead of racing.
 *
...
function complete_self_assessment(int $id): array
{
    ...
    try {
        ...
        $lock_stmt = $db->prepare("SELECT `status` FROM `self_assessments` WHERE `id` = :id FOR UPDATE");
        $lock_stmt->execute([':id' => $id]);
        $locked = $lock_stmt->fetch(PDO::FETCH_ASSOC);
        ...
        write_debug_log("complete_self_assessment failed for self_assessment id {$id}: " . $e->getMessage(), "error");
        db_close($db);
        return [];
    }
