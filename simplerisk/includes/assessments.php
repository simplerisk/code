 ...
 *
 * Deleting a pending risk requires submit_risks -- the same permission
 * required to push it to a real risk (promote_pending_risk_to_risk(), just
 * above). Without this gate, a caller holding only the 'assessments'
 * permission could destroy a pending-risk workflow record it has no
 * authority to act on.
 *
 * $self_assessment_only restricts the delete to rows generated by a
 * self-assessment (self_assessment_id IS NOT NULL). The self-assessment API's
 * delete route passes true so it can never reach a legacy pending risk
 * (self_assessment_id IS NULL) regardless of the id it's given; the legacy
 * promote/push flow above passes false (its default) since it must still be
 * able to consume a legacy pending risk after promoting it.
 *
 * @return bool true if the row was deleted (or, when self_assessment_only is
 *              set, existed and was in scope); false if the permission check
 *              failed or the scoped row did not exist.
 *********************************************/
function delete_pending_risk($pending_risk_id, $self_assessment_only = false)
{
    // Permission gate (same check/message as legacy push_pending_risk() /
    // promote_pending_risk_to_risk()).
    ...
    // When scoped to self-assessment-generated rows, confirm the row actually
    // belongs to a self-assessment before deleting anything.
    if ($self_assessment_only) {
        $stmt = $db->prepare("SELECT `id` FROM `pending_risks` WHERE `id`=:pending_risk_id AND `self_assessment_id` IS NOT NULL;");
