    ...
    else
    {
        ...
        foreach($risks as $row){
            ...
            $data = array(
                ...
                $escaper->escapeHtml($row['mitigation_percent']),
                ...
                $escaper->escapeHtml($row['comments'])
            );
                    ...
                    $html .= "<div class='control-block--row'>\n";
                        $html .= "<table width='100%'>\n";
                            $html .= "<tr>\n";
                                ...
                                $html .= "<td width='54%' colspan='3'>".$escaper->escapeHtml($control['short_name'])."</td>\n";
                            $html .= "<tr>\n";
                            ...
                            $html .= "<tr>\n";
                                ...
                                $html .= "<td width='20%'>".$escaper->escapeHtml($control['control_owner_name'])."</td>\n";
                            ...
                            $html .= "</tr>\n";
                            $html .= "<tr>\n";
                                $html .= "<td align='right'><strong>".$escaper->escapeHtml($lang['CurrentControlMaturity'])."</strong>: </td>\n";
                                $html .= "<td>".$escaper->escapeHtml($control['control_maturity_name'])."</td>\n";
                                $html .= "<td align='right'><strong>".$escaper->escapeHtml($lang['ControlClass'])."</strong>: </td>\n";
                                $html .= "<td width='22%'>".$escaper->escapeHtml($control['control_class_name'])."</td>\n";
                            $html .= "</tr>\n";
                            $html .= "<tr>\n";
                                $html .= "<td align='right'><strong>".$escaper->escapeHtml($lang['DesiredControlMaturity'])."</strong>: </td>\n";
                                $html .= "<td>".$escaper->escapeHtml($control['desired_maturity_name'])."</td>\n";
                                $html .= "<td width='13%' align='right'><strong>".$escaper->escapeHtml($lang['ControlPhase'])."</strong>: </td>\n";
                            ...
                            $html .= "<tr>\n";
                                ...
                                $html .= "<td colspan='3'>".nl2br($escaper->escapeHtml($control['description']))."</td>\n";
                            $html .= "</tr>\n";
                            $html .= "<tr>\n";
                        ...
                        $html .= "<div class='collapsible' style='".$panel_css."'>";
                            $html .= "<div class='row-fluid'>";
                                $html .= "<div class='span4'>
                                    ...
                                    <textarea class='active-textfield' title='".$escaper->escapeHtml($lang['Details']) ."' name='validation_details_".$control['id']."' style='width:100%;' rows='3'>".$validation["validation_details"]."</textarea>
                                </div>";
                                $html .= "</div>";
                                $html .= "<div class='row-fluid'>";
                                $html .= "<div class='span4'>
...
function initiateFrameworkControlTestsResponse()
{
    ...
    if (check_permission_compliance() && isset($_SESSION["initiate_audits"]) && $_SESSION["initiate_audits"] == 1)
    {
        $id     = (int)$_POST['id'];
        $type   = $_POST['type'];
...
function deleteTestAuditResponse()
{
    ...
    // If the user has compliance permissions
    if (check_permission_compliance() && isset($_SESSION["delete_audits"]) && $_SESSION["delete_audits"] == 1)
    {
...
function reopenTestAuditResponse()
{
    ...
    // If the user has compliance permissions
    if (check_permission_compliance() && isset($_SESSION["reopen_audits"]) && $_SESSION["reopen_audits"] == 1)
    {
        $audit_id = $_POST['id'];
            ...
            foreach($column_filters as $column_name => $val){
                switch ($column_name) {
                    ...
                    case "submission_date":
                        ...
                        break;
                    case "mitigation_planned":
                        if(($risk['mitigation_id']&&$val=="no")||(!$risk['mitigation_id']&&$val=="yes")){
                            $success = false;
                        }
                        break;
                    case "management_review":
...
function getTagOptionsOfType() {
    ...
    else
    {
        ...
        if ($type === 'risk' && !check_permission_riskmanagement()) {
            ...
            json_response(400, get_alert(true), NULL);
            return;
        } elseif(($type === 'questionnaire_answer' || $type === 'questionnaire_pending_risk') && (!check_permission_assessments() || !assessments_extra())) {
            set_alert(true, "bad", $lang['NoPermissionForAssessments']);
            json_response(400, get_alert(true), NULL);
            return;
        } elseif(($type === 'incident_management_source' || $type === 'incident_management_destination') && (!check_permission_incident_management() || !incident_management_extra())) {
...
function getTagOptionsOfTypes() {
    ...
    if (empty($types))
    {
        ...
        if (in_array('risk', $types) && !check_permission_riskmanagement()) {
            set_alert(true, "bad", $lang['NoPermissionForRiskManagement']);
            json_response(400, get_alert(true), NULL);
            return;
        } elseif(in_array('asset', $types) && !check_permission_asset()) {
            set_alert(true, "bad", $lang['NoPermissionForAsset']);
            json_response(400, get_alert(true), NULL);
            return;
        } elseif((in_array('questionnaire_answer', $types) || in_array('questionnaire_pending_risk', $types)) && (!check_permission_assessments() || !assessments_extra())) {
            set_alert(true, "bad", $lang['NoPermissionForAssessments']);
            json_response(400, get_alert(true), NULL);
            return;
        }
...
function getRiskCatalogDatatableAPI()
{
    ...
    if (is_admin())
    {
        ...
        foreach($risk_catalogs as $risk){
            $rows[] = array(
                "DT_RowId"=>$risk['id'],
                $escaper->escapeHtml($risk['grouping_name']),
                $escaper->escapeHtml($risk['number']),
...
function getThreatCatalogDatatableAPI()
{
    ...
    if (is_admin())
    {
        ...
        foreach($threat_catalogs as $threat){
            $rows[] = array(
                "DT_RowId"=>$threat['id'],
                $escaper->escapeHtml($threat['grouping_name']),
...
function my_open_risk_datatable() {
    ...
    foreach($risks as $risk){
        ...
        $data = array(
            ...
            planned_mitigation($risk['id'], $risk['mitigation_id']), // mitigation plan
            management_review(convert_id($risk['id']), $risk['mgmt_review'], $next_review) // management review
        );
        $success = true;
        foreach($manual_column_filters as $column_name => $val){
            if($column_name == "id") {
                ...
                if(($risk['mgmt_review']&&$val=="no")||(!$risk['mgmt_review']&&$val=="yes")){
                    $success = false;
                    break;
                }
            }
