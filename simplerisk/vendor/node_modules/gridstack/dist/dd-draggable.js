...
class DDDraggable extends DDBaseImplement {
    ...
    refreshHandles() {
        ...
        if (!wasDisabled)
            this.disable(true); // forDestroy=true skips the CSS class toggle
        ...
        this.dragEls = !handleName || this.el.classList.contains(handleName)
            ? [this.el]
            : (n?.subGrid ? [this.el.querySelector(this.option.handle) || this.el] : this.getAllHandles());
        if (this.dragEls.length === 0)
    ...
    _mouseDown(e) {
        ...
        if (e.isTrusted) {
            ...
            if (DDManager.mouseHandled && e.timeStamp !== DDManager.mouseHandledTimeStamp)
                delete DDManager.mouseHandled; // stale from incomplete prior drag
        }
