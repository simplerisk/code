...
export class Utils {
    ...
    static findInGrid(g, id, recursive) {
        ...
        for (const n of g.engine.nodes) {
            if (n.subGrid) {
                const nested = Utils.findInGrid(n.subGrid, id);
                if (nested)
                    return nested;
            }
        }
    ...
    static same(a, b) {
        ...
        if (typeof a !== 'object')
            return a == b; // intentional: coerces string/number pairs (e.g. gs-* attribute values)
    ...
    static removeInternalForSave(n, removeEl = true) {
        const nd = n;
        for (const key in nd) {
            if (key[0] === '_' || nd[key] === null || nd[key] === undefined)
                delete nd[key];
        }
        delete n.grid;
        if (removeEl)
