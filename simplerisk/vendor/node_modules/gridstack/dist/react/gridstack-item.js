...
import { useContext, useLayoutEffect, useMemo, useEffect, useRef, useState, } from "react";
...
function stableOptsKey(options) {
 ...
 */
export function GridStackItem({ id, options = {}, children, }) {
    ...
    const widgetCtx = useMemo(() => {
        ...
        return {
            id,
            registerSerializer: (serialize, deserialize) => registerWidgetSerializer(id, serialize, deserialize),
        };
    ...
    useLayoutEffect(() => {
        if (!grid)
            return;
        const node = Utils.findInGrid(grid, id, true);
        const cont = node?.el?.querySelector(".grid-stack-item-content") ?? null;
        ...
        return () => {
            ...
            Promise.resolve().then(() => {
                if (!remountedRef.current)
                    return; // setup re-ran (Strict Mode) — cancel
                if (!grid.engine)
                    return;
                const node = Utils.findInGrid(grid, id, true);
                if (!node?.el || !node.grid)
                    return;
    ...
    if (!grid || !container || children == null)
        return null;
    return (_jsx(GridStackWidgetContext.Provider, { value: widgetCtx, children: createPortal(children, container) }));
}
