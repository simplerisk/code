...
class GridStack {
    ...
    destroy(removeDOM = true) {
        if (!this.el)
            return this; // prevent multiple calls
        this.offAll();
    ...
    updateOptions(o) {
        ...
        if (o.row !== undefined) {
            ...
            if (this.engine.getRow() > o.row)
                this.compact(); // relayout widgets that exceed new row limit
        }
        else {
    ...
    rotate(els, relative) {
        GridStack.getElements(els).forEach(el => {
            ...
            const rotRec = rot;
            Object.keys(rotRec).forEach(k => { if (rotRec[k] === undefined)
                delete rotRec[k]; });
            const _orig = n._orig;
            this.update(el, rot);
    ...
    onResize(clientWidth = this.el?.clientWidth) {
        if (!clientWidth)
            return this; // return if we're gone or no size yet (will get called again)
