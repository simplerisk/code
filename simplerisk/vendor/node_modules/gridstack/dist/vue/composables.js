...
export function useGridStack() {
    ...
    return {
        ...
        removeWidget(el, removeDOM, triggerEvent) {
 ...
 */
export function useGridStackItem() {
 ...
 * ```
 */
export function useWidgetSerializer(opts) {
    const itemCtx = inject(GS_ITEM_CONTEXT_KEY);
    onMounted(() => {
        if (!itemCtx?.registerSerializer)
            return;
        const cleanup = itemCtx.registerSerializer(() => opts.serialize?.(), opts.deserialize ? (data) => opts.deserialize?.(data) : undefined);
