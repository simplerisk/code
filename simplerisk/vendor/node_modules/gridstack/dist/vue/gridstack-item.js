import { defineComponent, h, inject, onBeforeUnmount, onMounted, provide, ref, Teleport, watch, } from 'vue';
...
export const GridStackItem = defineComponent({
    ...
    setup(props, { slots }) {
        ...
        if (!ctx)
            throw new Error('<GridStackItem> must be used inside <GridStack>');
        ...
        onMounted(() => {
            ...
            if (!node?.el) {
                ...
                g.addWidget({ ...props.options, id: props.id });
            }
            else if (node.grid === g) {
                // Already in the outer grid — update position/opts.
                g.update(node.el, { ...props.options, id: props.id });
            }
        ...
        const cleanupParentSerializer = ctx.registerWidgetSerializer(props.id, () => serializeFn?.(), (data) => deserializeFn?.(data));
        onBeforeUnmount(() => {
            alive = false;
        ...
        function registerSerializer(serialize, deserialize) {
