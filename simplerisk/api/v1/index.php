<?php
    ...
    // Gate: API v1 is deprecated. When disabled by the administrator, refuse all
    // requests routed through this entry point — including direct /api/v1/<path>
    // requests. The unversioned /api/<path> dispatcher (../index.php) skips this
    // file entirely when the setting is off and falls through to v2, so this
    // check only fires for explicit /api/v1/<path> callers.
    if (!get_setting('enable_api_v1'))
    {
        json_response(
            410,
            "API v1 is deprecated and disabled on this instance. Please migrate your integration to /api/v2/. See /api/v2/documentation/ for the supported API surface.",
            null
        );
        return;
    }
