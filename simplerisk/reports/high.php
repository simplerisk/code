...
render_header_and_sidebar(['datatables', 'chart.js']);
...
<div class="row bg-white">
    <div class="col-12 m-2">
        <div class="row">
            <div class="col-6">
                <label><?php echo $escaper->escapeHtml($lang['RiskScoreUsing']); ?></label>
                <form method="GET">
                    <select name="score_used" id="score_used_dropdown" class="form-field form-select" onchange="submit();">
                        <option value='inherent' name="" <?php if ($score_used !== "residual") echo 'selected'; ?>><?php echo $escaper->escapeHtml($lang['InherentRisk']);?></option>
            ...
            <div class="col-6">
                <?php open_risk_level_pie($escaper->escapeHtml($lang['RiskLevel']), "open_risk_level_pie", false, $score_used); ?>
            </div>
        </div>
        <div class="row mb-2">
            <div class="col-12">
                    ...
                    if (($next_review_date_uses === 'ResidualRisk' ? 'residual' : 'inherent') !== $score_used) {
                        ...
                        echo "<div class='text-danger'>" . $escaper->escapeHtml($warning) . "</div>";
                    }
                ?>
            </div>
        </div>
        <div class="row">
            <div class="col-12">
                <?php get_high_risk_report_table(); ?>
            </div>
        </div>
    </div>
</div>
