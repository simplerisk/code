<?php
    ...
    * file, You can obtain one at http://mozilla.org/MPL/2.0/. */
    ...
    if (!isset($_GET['frameworks'])) {
        ...
        $filter_ids      = $selected_fw_ids;
    }

    // Fetch remediation cycle time data
    $cycle_time_data = get_audit_remediation_cycle_time_by_framework($filter_ids);
        ...
        <div class="card-body border my-2">
            ...
            <div class="row mb-3">
                <div class="col-4">
                    ...
                    <form id="audit_filter_form" method="GET">
                        <input type="hidden" name="frameworks" id="framework_options" value="<?= $escaper->escapeHtml($current_filter_value); ?>">
                    </form>
                </div>
            </div>

            <strong><?= $escaper->escapeHtml($lang['AuditRemediationCycleTimeDescription']); ?></strong>

            <?php if (empty($cycle_time_data)): ?>
                <div class="alert alert-info mt-3"><?= $escaper->escapeHtml($lang['NoAuditDataAvailable']); ?></div>
            <?php else: ?>
            <div class="mt-4">
                <?php
...
$(function() {
    $('#frameworks_select').multiselect({
        allSelectedText: '<?= js_string_escape($lang['AllFrameworks']); ?>',
        buttonWidth: '100%',
